...
methods on the Adapter object which redirects them into calls to the legacy component. This strategy can be implemented either with 
...
This first version creates a pure abstract base class that acts as an interface. A second class that acts as an interface but not pure.
A third class that inherits/implements both interfaces, but wants to execute by the method of the first the method of the second. */
...
public:
    LegacyRectangle(const Coordinate x1, const Coordinate y1, const Coordinate x2, const Coordinate y2)
...
public:
    RectangleAdapter(const Coordinate x, const Coordinate y, const Dimension w, const Dimension h) 
