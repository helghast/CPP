...
// int main ()  // Linux users should use this line instead the previous one
{
	...
	while (!mIO.IsKeyDown (SDLK_ESCAPE))
	{
		...
		switch (mKey)
		{
			...
			case SDLK_SPACE:
			{
				const int pRotation = (mGame.mRotation + 1) % 4;
				if (mBoard.IsPossibleMovement(mGame.mPosX, mGame.mPosY, mGame.mPiece, pRotation))
					mGame.mRotation = pRotation;
				break;
			}
		}

		// ----- Vertical movement -----
		const unsigned long mTime2 = SDL_GetTicks();
